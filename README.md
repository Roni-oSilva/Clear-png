...
│   │   ├── ai/matting.py        # Visão computacional leve: histerese, componentes, filtro guiado, pincel
│   │   ├── ai/pipeline.py       # Abrir/compor imagens com pouca RAM + fluxo de refinamento por pincel
...
│   ├── assets/editor.js         # Editor por pincel (remover / restaurar), pré-visualização local
...
### `POST /api/refine-background` — refinamento por pincel

`multipart/form-data`

| Campo        | Tipo     | Descrição |
|--------------|----------|-----------|
| `file`       | ficheiro | A imagem **original** (a mesma enviada para remover o fundo) |
| `strokes`    | texto    | JSON com os traços (ver abaixo) |
| `mask`       | ficheiro | Opcional. PNG em tons de cinza com o recorte da 1.ª passagem (≤1024 px). Com ele a IA **não** volta a correr |
| `snap_edges` | boolean  | Padrão `true`. Alinha as bordas editadas aos contornos reais da imagem |

```json
{ "strokes": [
  { "mode": "erase",   "r": 0.02, "pts": [0.62, 0.18, 0.90, 0.18] },
  { "mode": "restore", "r": 0.01, "pts": [0.30, 0.62, 0.40, 0.52, 0.46, 0.46] }
] }
```

- Coordenadas `pts` normalizadas em 0..1 (x, y, x, y, …) sobre a imagem já orientada; `r` = raio do pincel em fração do lado maior.
- Os traços aplicam-se **por ordem** (um "restaurar" por cima de um "remover" ganha).
- `erase` que cobre ≥35% de uma mancha solta remove a mancha **inteira**; num objeto grande, só apaga a área pintada.
- `restore` recupera a área pintada **e** as partes finas/fracas ligadas a ela (ex.: a ponta de uma cerda).
- Limites: 600 traços, 60 000 coordenadas. Erros: `invalid_strokes`, `no_strokes`, `invalid_mask`, `refine_failed`.

Resposta igual à de `/api/remove-background` (PNG RGBA + cabeçalhos), mais `X-Strokes-Applied`.

```bash
curl -F "file=@foto.jpg" -F "mask=@mascara.png;type=image/png" \
     -F 'strokes={"strokes":[{"mode":"erase","r":0.02,"pts":[0.1,0.1,0.3,0.1]}]}' \
     http://localhost:8000/api/refine-background -o refinado.png
```

### `GET /api/health`
Estado do servidor, modelo ativo e limites (usado pelo frontend para sincronizar o tamanho máximo).

## Precisão em objetos complexos (motor lite)

O pós-processamento em `matting.py` corre a ≤1 MP, só com NumPy + Pillow:

1. **TTA de espelho** — a IA vê a imagem normal e invertida; a média reduz falhas pontuais.
2. **Histerese + reconstrução geodésica** — zonas pouco confiantes só ficam se estiverem **ligadas** ao objeto
   (preserva cerdas, pontas e hastes finas; descarta ruído solto).
3. **Componentes conexos** — remove manchas pequenas isoladas, mas mantém objetos separados relevantes.
4. **Filtro guiado a cores** numa faixa estreita à volta da borda — a máscara cola aos contornos reais.

Medido numa cena sintética com verdade conhecida (pincel com 70 cerdas finas + chave de fendas, fundo texturado):

| | Antes | Agora |
|---|---|---|
| IoU do recorte | 0.642 | **0.798** |
| Partes finas preservadas (cerdas, haste) | 84.8% | **99.8%** |
| Fundo mantido por engano | 0.60% | 0.93% |
| RAM máxima do servidor (Render) | ~370 MB | ~340 MB |

A previsão com espelho duplica o tempo da IA (+~0,4 s aqui); desligue com `LITE_TTA=false` se precisar de velocidade.

## Editor por pincel

No resultado, **Editar com pincel** abre o editor:
- **Remover** (vermelho) e **Restaurar** (verde), tamanho ajustável, desfazer/refazer, "ver original por baixo".
- A pré-visualização é imediata no navegador; **Aplicar**/**Concluir** envia para `/api/refine-background`,
  que alinha as bordas e aplica as regras inteligentes (manchas inteiras, partes finas ligadas).
- Pedido típico: ~70 KB (imagem pequena) — os traços são vetores (centenas de bytes) e a máscara base ~20–120 KB.
- Funciona com rato, caneta e toque; também em imagens do lote (a miniatura e o ZIP usam a versão editada).
...
| `LITE_TTA`               | `true`              | Só lite: previsão também na imagem espelhada (mais precisa, ~2x tempo de IA) |
...
- Editor por pincel para remover/restaurar áreas manualmente, com refinamento no servidor
