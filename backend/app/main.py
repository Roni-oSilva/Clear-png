...
app.add_middleware(
    ...
    allow_headers=["*"],
    expose_headers=["X-Processing-Time", "X-Image-Width", "X-Image-Height", "X-Refine-Applied"],
)
...
async def remove_background(
    ...
    # 1) Receção em streaming para disco + validação (apaga tudo se falhar)
    try:
        await save_upload_limited(file, input_path, settings.max_upload_bytes)
        info = await run_in_threadpool(validate_image_file, input_path, settings.max_pixels)
    except BaseException:
        storage.remove(job_dir)
        raise

    refine = refine_edges and info.width * info.height <= settings.max_refine_pixels

    # 2) Inferência
    try:
        async with job_semaphore:  # limita inferências simultâneas (CPU/RAM)
