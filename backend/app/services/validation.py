...
def validate_image_file(
    path: Path, max_pixels: int, max_decode_pixels: int | None = None, max_webp_pixels: int | None = None
) -> ImageInfo:
    ...
    try:
        ...
        with Image.open(path) as img:
            ...
            limit = max_webp_pixels if fmt == "WEBP" and max_webp_pixels else max_decode_pixels
            if limit and fmt in ("PNG", "WEBP") and width * height > limit:
                raise ImageValidationError(
                    f"{'WebP' if fmt == 'WEBP' else 'PNG'} de {width}×{height} ({width * height / 1e6:.0f} MP) é grande "
                    f"demais para este servidor (máx. {limit / 1e6:.0f} MP neste formato). "
                    "Envie em JPG — aceita até 100 MB — ou reduza a resolução.",
                    status_code=413,
                    code="too_many_pixels",
                )
            img.verify()
        ...
        with Image.open(path) as img:
            if img.format == "JPEG":
                img.draft("RGB", (2048, 2048))  # verifica o ficheiro todo sem o descodificar em tamanho real
            img.load()
