...
class BackgroundRemover:
    ...
    def predict_alpha(self, work: Image.Image) -> "np.ndarray":
        """Máscara float [0,1] na resolução de `work` (usada pelo refinamento por pincel)."""
        import numpy as np

        masks = self._get_session().predict(work.convert("RGB"))
