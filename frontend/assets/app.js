...
(() => {
  ...
  async function checkServer() {
    ...
    try {
      const res = await fetch(`${CONFIG.apiBase}/api/health`, { cache: 'no-store' });
  ...
  function statusText(item) {
    switch (item.status) {
      ...
      case 'done': {
        ...
        return `✓ ${item.width}×${item.height}px${t}`;
      }
  ...
  function updateItem(item) {
    ...
    } else if ((item.status === 'error' || item.status === 'cancelled') && item.url && !state.busy) {
  ...
  async function runBatch(files) {
    ...
    await Promise.all(state.items.map(async (item) => {
      try { await prepare(item); } catch (err) { item.status = 'error'; item.error = err.message; }
      updateItem(item);
    }));
    updateBatchTotals();
  ...
  function openResult(item) {
    ...
    const parts = [item.name, `${item.width}×${item.height}px`, formatBytes(item.resultBlob.size)];
