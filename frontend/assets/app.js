...
(() => {
  ...
  function toast(message, type = 'error', ms = 5000) {
    const svg = (d) => `<svg class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${d}</svg>`;
    const icons = {
      error: svg('<circle cx="12" cy="12" r="10"/><path d="M12 8v5M12 16.5v.5"/>'),
    ...
    node.innerHTML = `<span class="toast-icon">${icons[type]}</span><p class="flex-1"></p>
      <button type="button" class="text-slate-400 hover:text-slate-700 dark:hover:text-white" aria-label="Fechar">
  ...
  function requestRemoval(item, sim, refine, opts = {}) {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      item.xhr = xhr;
      xhr.open('POST', `${CONFIG.apiBase}${opts.endpoint || '/api/remove-background'}`);
      ...
      if (opts.fields) {
        for (const [k, v] of Object.entries(opts.fields)) {
          if (v instanceof Blob) fd.append(k, v, `${k}.png`);
          else fd.append(k, v);
        }
      } else {
  ...
  function applyResult(item, res) {
    item.aiBlob = res.blob;          // recorte original da IA (base para o pincel)
    item.baseMask = null;
    item.edit = null;
    item.edited = false;
  ...
  function statusText(item) {
    switch (item.status) {
      ...
      case 'done': {
        ...
        return `${item.edited ? 'Editada · ' : ''}${item.outWidth}×${item.outHeight}px${item.downscaled ? ' (reduzida)' : ''}${t}`;
      }
  ...
  function backToBatch() {
    if (state.mode !== 'batch' || window.Editor?.isBusy()) return;
    window.Editor?.close(true);
    showView('batch');
    updateBatchTotals();
  }

  function resetAll() {
    if (state.busy || window.Editor?.isBusy()) return;
    window.Editor?.close(true);
    clearAll();
    showView('upload');
    orb('setMode', 'idle');
  ...
  function openResult(item) {
    ...
    if (item.edited) parts.push('editada');
  ...
  document.addEventListener('keydown', (e) => {
    if (window.Editor?.isOpen()) {
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's' && state.current) { e.preventDefault(); downloadItem(state.current); }
      return; // o editor trata os seus atalhos; Esc não descarta edições por engano
    }
    if (e.key === 'Escape') {
  ...
  // ------------------------------------------------------------------ Editor por pincel (ponte com editor.js)
  // Máscara base = canal alfa do recorte da IA, reduzido a ≤1024 px e enviado como PNG cinzento.
  // Com ela, o servidor não precisa de voltar a correr a IA (menos CPU e RAM no Render).
  async function getBaseMask(item) {
    if (item.baseMask) return item.baseMask;
    const bmp = await createImageBitmap(item.aiBlob);
    ...
    item.baseMask = await new Promise((r) => c.toBlob(r, 'image/png'));
    return item.baseMask;
  }

  async function refineRequest(item, strokesJson, maskBlob, onProgress) {
    const sim = new Sim((v) => onProgress?.(v));
    state.busy = true;
    try {
      const res = await requestRemoval(item, sim, true, {
        endpoint: '/api/refine-background',
        fields: { strokes: strokesJson, mask: maskBlob, snap_edges: 'true' },
      });
      sim.set(100);
      return res;
    } finally {
  ...
  function setItemResult(item, blob) {
    ...
    item.edited = blob !== item.aiBlob;
    updateItem(item);
  }

  async function commitEdit(item, res) {
    setItemResult(item, res.blob);
    if (res.outWidth) { item.outWidth = res.outWidth; item.outHeight = res.outHeight; }
    await loadImage(item.resultUrl);
  }

  async function restoreResult(item, blob) {
    if (!blob || blob === item.resultBlob) return;
    setItemResult(item, blob);
    await loadImage(item.resultUrl);
  }

  function refreshResultView(item) {
    if (!item || state.current !== item) return;
    el.afterImg.src = el.sideAfter.src = el.resultOnlyImg.src = item.resultUrl;
    const parts = [item.name, `${item.outWidth}×${item.outHeight}px${item.downscaled ? ' (reduzida)' : ''}`, formatBytes(item.resultBlob.size)];
    if (item.edited) parts.push('editada');
    el.meta.textContent = parts.join(' · ');
  ...
  window.ClearCutApp = {
    ...
    commitEdit,
    restoreResult,
    onEditorClose: refreshResultView,
  };

  $('#edit-btn').addEventListener('click', () => {
    if (!state.current) return;
    if (state.busy) return toast('Aguarde: ainda há imagens em processamento.', 'info');
    window.Editor?.open(state.current);
  });
